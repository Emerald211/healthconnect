...
func (r *PatientRepository) Create(ctx context.Context, p domain.Patient, hashedPassword string) (domain.Patient, error) {
	...
	query := `
		...
		RETURNING id, name, email, phone, date_of_birth, gender, address, is_active, created_at, updated_at
	`
...
func (r *PatientRepository) FindByEmail(ctx context.Context, email string) (domain.Patient, string, error) {
	query := `
		SELECT id, name, email, phone, password, date_of_birth, gender, address, is_active, created_at, updated_at
		FROM patients
		WHERE email = $1
	`
...
func (r *PatientRepository) FindByID(ctx context.Context, id string) (domain.Patient, error) {
	...
	query := `
			SELECT id, name, email, phone, date_of_birth, gender, address, is_active, created_at, updated_at
			FROM patients
			WHERE id = $1
		`
